...
@Slf4j
@Component
public record LemmaBuilder(LemmaConfiguration config) {
    ...
    public List<String> getLemma(String word) throws IOException {
        ...
        if (checkLanguage(word).equals("Russian")) {
            List<String> baseRusForm = config.russianLuceneMorphology().getNormalForms(word);
    ...
    private boolean isCorrectWordForm(String word) throws IOException {
        List<String> morphForm = config.russianLuceneMorphology().getMorphInfo(word);
