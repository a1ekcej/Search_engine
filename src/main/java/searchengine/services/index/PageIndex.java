...
public class PageIndex extends RecursiveTask<List<PageDto>> {
    ...
    protected List<PageDto> compute() {
        try {
            ...
            Elements elements = doc.select("body")
                    .select("a");
            List<PageIndex> taskList = new ArrayList<>();
            String link;
            PageIndex task;
            for (Element el : elements) {
                link = el.attr("abs:href");
                if (isElementsTypeBool(link)
                        && link.startsWith(el.baseUri())
                        && !link.equals(el.baseUri())
                        && !link.contains("#")
                        && !urlList.contains(link)) {
                    urlList.add(link);
                    task = new PageIndex(link, urlList, pageDtoList, config);
                    task.fork();
                    taskList.add(task);
                }
            }
    ...
    private boolean isElementsTypeBool(String pathPage) {
